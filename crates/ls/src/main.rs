...
fn ls(dir: String, options: &Vec<LsArg>) {
    ...
    let _ = read_dir(dir).unwrap().for_each(|file| {
...
fn print_dir_entry(entry: DirEntry, options: &Vec<LsArg>) -> (Option<String>, Option<String>) {
    let file_name = entry.file_name().into_string().unwrap().replace("\"", "");
    let is_dir = entry.file_type().unwrap().is_dir();

    let binding = {
        let color = if is_dir {
            ansi_term::Colour::Blue.normal()
        } else {
            ansi_term::Colour::White.normal()
        };
    ...
    options.iter().for_each(|x| match x {
